...
describe('Player Ability Progression & Unlocks', () => {
  test('default new progress includes doubleJump and swim unlocked by default', () => {
    const progress = createDefaultProgress('knight');
    assert.deepEqual(progress.unlockedAbilities, ['doubleJump', 'swim']);
    assert.equal(hasAbility(progress, 'doubleJump'), true);
    assert.equal(hasAbility(progress, 'swim'), true);
  });

  test('official new profile progress includes doubleJump and swim', () => {
  ...
  test('sanitizePlayerAbilities removes duplicates and invalid items', () => {
    const progress = createDefaultProgress('knight');
    progress.unlockedAbilities = ['doubleJump', 'doubleJump', 'invalidAbility' as unknown as 'doubleJump', 'swim'];
